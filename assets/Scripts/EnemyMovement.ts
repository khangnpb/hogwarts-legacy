...
export class EnemyMovement extends Enemy {
  ...
  checkIsNear(deltaTime: number) {
    ...
    let distanceWithOriginPos = Math.sqrt(
      (this.originPosX - ex) * (this.originPosX - ex) +
        (this.originPosY - ey) * (this.originPosY - ey)
    );

    if (distanceWithOriginPos <= 50) this.isGoingBack = false;

    if (distanceWithOriginPos >= 400) this.isGoingBack = true;

    if (this.isGoingBack) return 0;

    if (distance < 230) return 1;
    return 2;
  }
  ...
  move(deltaTime: number) {
    ...
    switch (
      this.checkIsNear(deltaTime) //0: di ve vi tri cu, 1: di den player, 2: dung yen
    ) {
      case 0:
        isGoLeft = this.goToPosition(
          this.originPosX,
          this.originPosY,
          deltaTime
        );
